 ...
 * @author vuqua
 */
public class WorkerDAOCa2 {
       public List<Worker> getAllWorkers(String idCongNhan) {
    List<Worker> workers = new ArrayList<>();

    try (Connection connection = DatabaseConnection.connect()) {
        String query = "SELECT * FROM Worker WHERE idca = 2 AND macongnhan = ?";
    ...
    try (Connection connection = DatabaseConnection.connect()) {
        String query = "UPDATE Worker SET checkout = ?, thoigianlam = ? WHERE ngaylam = ? AND macongnhan = ? AND idca = 2";
        try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
        ...
        try (Connection connection = DatabaseConnection.connect()) {
            ...
            String query = "SELECT * FROM Worker WHERE ngaylam = ? AND macongnhan = ? AND idca = 2";
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
